    ...
    methods:{ 
      openModel(funct,id){
            ...
            if (this.validForm()) {
                ...
                const responseSave = await axios.post('../php/bd_question.php', {  action:action, model: this.questionSelected}).then(function(response){ return  response });
            ...
            const responseDelete = await axios.post('../php/bd_question.php', {  action:'delete',id: id, model: this.questionSelected}).then(function(response){ return  response });
            console.log(responseDelete);   
            application_poll.manageError(responseDelete,"Data Deleted",'Eliminado.');  
            await this.loadModelQuestion();  
        },async loadModelQuestion(){
            const questions = await axios.post('../php/bd_question.php', {  action:'fetchall',id_encuesta: this.poll.id }).then(function(response){ return  response.data });
...
// ================================================================================================
        async showOpciones(id){ 
            ...
            for (let index = 0; index < this.options.length; index++) {
                ...
                const result  = await axios.post("../php/bd_option.php", { action: element.action ,  model: element }).then(function (response) {    return response.data;   })  .catch(function (response) {    return response.data;  }) 
                try {
    ...
    created:async function(){
        ...
        if (id_encuesta > 0) {
            const respuesta = await axios.post('../php/bd_poll.php', {  action:'fetchSingle',id: id_encuesta }).then(function(response){ return  response.data });
            this.poll = respuesta;  
            await this.loadModelQuestion(); 
            const tiposResponce = await axios.post('../php/bd_question.php', {  action:'getTipos'}).then(function(response){ return  response.data });
